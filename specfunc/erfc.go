...
func erfcasympsum(x float64) float64 {
	e := 1.0
	coef := 1.0
	for i := 2; i < 5; i++ {
		coef *= -float64(2*i+1) / (float64(i) * (4 * x * x * x * x))
		e += coef
	}
	return e
}
...
func erfseries(x float64, result *Result) error {
	coef := x
	e := coef
	var del float64
	for k := 2; k < 30; k++ {
	...
	result.val = 2.0 / gsl.M_SQRTPI * e
	result.err = 2.0 / gsl.M_SQRTPI * (math.Abs(del) + gsl.DBL_EPSILON)
	return err.SUCCESS
}

/* Chebyshev fit for erfc((t+1)/2), -1 < t < 1
 */
var erfc_xlt1_data = []float64{
...
/* Chebyshev fit for erfc(x) math.Exp(x^2), 1 < x < 5, x = 2t + 3, -1 < t < 1
 */
var erfc_x15_data = []float64{
...
/* Chebyshev fit for erfc(x) x math.Exp(x^2), 5 < x < 10, x = (5t + 15)/2, -1 < t < 1
 */
var erfc_x510_data = []float64{
...
func erfc_asymptotic(x float64) float64 {
	return math.Exp(-x*x) / x * erfcasympsum(x) / gsl.M_SQRTPI
}

func log_erfc_asymptotic(x float64) float64 {
	return math.Log(erfcasympsum(x)/x) - x*x - LogRootPi_
}

/*-*-*-*-*-*-*-*-*-*-*-* Functions with Error Codes *-*-*-*-*-*-*-*-*-*-*-*/

func Erfc_e(x float64, result *Result) error {
	ax := math.Abs(x)
	...
	if ax <= 1.0 {
		...
		e_val = ex2 * c.val
		e_err = ex2 * (c.err + 2.0*math.Abs(x)*gsl.DBL_EPSILON)
	} else if ax < 10.0 {
		...
		e_val = exterm * c.val
		e_err = exterm * (c.err + 2.0*math.Abs(x)*gsl.DBL_EPSILON + gsl.DBL_EPSILON)
	} else {
		e_val = erfc8(ax)
		e_err = (x*x + 1.0) * gsl.DBL_EPSILON * math.Abs(e_val)
	}
...
func Log_erfc_e(x float64, result *Result) error {
	if x*x < 10.0*gsl.ROOT6_DBL_EPSILON {
		y := x / gsl.M_SQRTPI
		/* series for -1/2 Log[Erfc[Sqrt[Pi] y]] */
		c3 := (4.0 - gsl.M_PI) / 3.0
		c4 := 2.0 * (1.0 - gsl.M_PI/3.0)
...
func Erf_e(x float64, result *Result) error {
	if math.Abs(x) < 1.0 {
...
func Erf_Z_e(x float64, result *Result) error {
	ex2 := math.Exp(-x * x / 2.0)
	result.val = ex2 / (gsl.M_SQRT2 * gsl.M_SQRTPI)
	result.err = math.Abs(x*result.val) * gsl.DBL_EPSILON
	result.err += 2.0 * gsl.DBL_EPSILON * math.Abs(result.val)
	CheckUnderflow(result)
	return err.SUCCESS
}

func Erf_Q_e(x float64, result *Result) error {
	result_erfc := new(Result)
	stat := Erfc_e(x/gsl.M_SQRT2, result_erfc)
...
func Erfc(x float64) (float64, error) {
	result := new(Result)
	status := Erfc_e(x, result)
	return result.val, status
}

func Log_erfc(x float64) (float64, error) {
	result := new(Result)
	status := Log_erfc_e(x, result)
	return result.val, status
}

func Erf(x float64) (float64, error) {
	result := new(Result)
	status := Erf_e(x, result)
	return result.val, status
}

func Erf_Z(x float64) (float64, error) {
	result := new(Result)
	status := Erf_Z_e(x, result)
	return result.val, status
}

func Erf_Q(x float64) (float64, error) {
	result := new(Result)
	status := Erf_Q_e(x, result)
	return result.val, status
}

func Hazard(x float64) (float64, error) {
	result := new(Result)
